...
local LSTM = require 'misc.LSTM_baby_atten'

local attention = require 'misc.attention_88'
local img_embedding = require 'misc.img_embedding_88_atten'
...
function layer:sample(inputs, opt)
  ...
  local img_input = fc
  local fc_feat, fc_feat_embed = unpack(self.img_embedding:forward(img_input))
  ...
  for t=1,self.seq_length+2 do
    ...
    if t > 1 then
      local atten_input = {h_out, p_out, fc_feat, fc_feat_embed}
...
function layer:sample_beam(inputs, opt)
  ...
  for k=1,batch_size do
    ...
    local done_beams = {}
    local fc_feat_k = fc_feat[{ {k,k} }]:expand(beam_size, self.input_encoding_size) -- k'th image feature expanded out
    local fc_feat_embed_k = fc_feat_embed[{ {k,k} }]:expand(beam_size, self.input_encoding_size) -- k'th image feature expanded out
    ...
    for t=1,self.seq_length+1 do
      ...
      local h_out = out[self.num_state+1]
      local p_out = out[self.num_state+2]
      local atten_input = {h_out, p_out, fc_feat_k, fc_feat_embed_k}
      logprobs = self.attention:forward(atten_input)
...
function layer:updateOutput(input)
  ...
  self.img_input = fc
  self.fc_feat,self.fc_feat_embed = unpack(self.img_embedding:forward(self.img_input))
  ...
  for t = 1,self.seq_length+2 do
    ...
    if not can_skip then
      ...
      if t > 1 then
        self.atten_inputs[t] = {h_out, p_out, self.fc_feat, self.fc_feat_embed}
        local atten_out = self.attentions[t]:forward(self.atten_inputs[t])
        self.output:narrow(1,t-1,1):copy(atten_out)
        self.tmax = t
      end
    end
  end
...
function layer:updateGradInput(input, gradOutput)
  local dfc_embed, dfc-- grad on input images
  ...
  for t=self.tmax,2,-1 do
    -- print("updateGradInput   t-:::::::::::::::::::::::::",t)

    local d_atten = self.attentions[t]:backward(self.atten_inputs[t], gradOutput[t-1])
    if not dfc then dfc = d_atten[3] else dfc:add(d_atten[3]) end
    if not dfc_embed then dfc_embed = d_atten[4] else dfc_embed:add(d_atten[4]) end
    ...
    for k=2,self.num_state+2 do table.insert(dstate, dinputs[k]) end
  ...
  local dimgs_cnn = self.img_embedding:backward(self.img_input, {dfc, dfc_embed})

  self.gradInput = {dimgs_cnn}
...
function crit:updateOutput(input, seqandvocab)
  local seq = seqandvocab[1]
  --local seq_len = inputs[3]
  local vocab = seqandvocab[2]
