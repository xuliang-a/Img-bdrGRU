...
function GRU.gru(input_size, output_size, rnn_size, n, dropout)
  ...
  for L = 1,n do
    ...
    local rlt = nn.CAddTable()({zh,zhm1})
    local mlt = nn.Linear(rnn_size,rnn_size)(rlt)
    local xt_h
    if input_size_L == rnn_size then 
      xt_h = x
    else
      xt_h = nn.Linear(input_size_L,rnn_size)(x)
    end
    local next_h = nn.CAddTable()({mlt,xt_h})
    --local next_h = nn.ReLU()(no_transfer)
    --local next_h = nn.BatchNormalization()(relu_res)
