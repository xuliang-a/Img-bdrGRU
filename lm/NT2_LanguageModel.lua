require 'nn'
require 'lm.LookupTableMaskZero'
local utils = require 'utils.utils'
local net_utils = require 'utils.net_utils'
local LSTM = require 'lm.NT2_LSTM'

local img_embedding = require 'lm.NT2_img_embedding'
...
function layer:sample(inputs, opt)
  ...
  local img_input = fc
  local fc_feat = self.img_embedding:forward(img_input)
  ...
  for t=1,self.seq_length+2 do
    ...
    --print(p_out)
    --print("t",t)
    -- local atten_input = {h_out, p_out, conv_feat, conv_feat_embed,prev_h}
...
function layer:sample_beam(inputs, opt)
  ...
  for k=1,batch_size do
    ...
    local done_beams = {}
    local imgk = fc_feat[{ {k,k} }]:expand(beam_size, self.input_encoding_size) -- k'th image feature expanded out
...
function layer:updateOutput(input)
  ...
  self.output:resize(self.seq_length+2, batch_size, self.vocab_size+1):zero()

  self.img_input = fc
  self.fc_feat  = self.img_embedding:forward(self.img_input)
...
function layer:updateGradInput(input, gradOutput)
  local dfc-- grad on input images
  ...
  for t=self.tmax,1,-1 do
    ...
    for k=2,self.num_state+1 do table.insert(dstate, dinputs[k]) end
    if t == 1 then
      dimgs = dxt
    else
  ...
  local  dfc = self.img_embedding:backward(self.img_input, dimgs)
...
function crit:updateOutput(input, seqandvocab)
  ...
  local D = seq:size(1)
  assert(D == L-2, 'input Tensor should be 1 larger in time')
  ...
  local s, it
  s, it = torch.max(input, 3)
  it:resize(L,N)
  local sent_ture =net_utils.decode_sequence(vocab, seq)
  local sent_prev =net_utils.decode_sequence(vocab, it:narrow(1,2,L-1))
  for k=1,#sent_prev
  do
    print('k=', k)
    print(string.format('=======>sent_prev : %s', sent_prev[k]))
    print(string.format('=======>sent_ture : %s', sent_ture[k]))
  end
