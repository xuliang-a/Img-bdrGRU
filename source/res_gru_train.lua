...
require 'loadcaffe'
-- local imports
local utils = require 'utils.NT2_ORG_utils'
require 'utils.NT2_ORG_DataLoader'
require 'lm.RES_GRU_LanguageModel'
local net_utils = require 'utils.NT2_ORG_net_utils'
require 'utils.NT2_ORG_optim_updates'
...
-- Data input settings
-- full coco dataset
cmd:option('-input_h5','/home/zt/ai/dataset/coco/cocotalk.h5','path to the h5file containing the preprocessed dataset')
cmd:option('-input_json','/home/zt/ai/dataset/coco/cocotalk.json','path to the json file containing additional info and vocab')

-- own test dataset
-- cmd:option('-input_h5','/home/zt/ai/dataset/coco/own_dataset/cocotalk.h5','path to the h5file containing the preprocessed dataset')
-- cmd:option('-input_json','/home/zt/ai/dataset/coco/own_dataset/cocotalk.json','path to the json file containing additional info and vocab')

cmd:option('-cnn_proto','../pre_trained/cnn/vgg16/VGG_ILSVRC_16_layers_deploy.prototxt','path to CNN prototxt file in Caffe format. Note this MUST be a VGGNet-16 right now.')
cmd:option('-cnn_model','../pre_trained/cnn/vgg16/VGG_ILSVRC_16_layers.caffemodel','path to CNN model file containing the weights, Caffe format. Note this MUST be a VGGNet-16 right now.')
...
cmd:option('-checkpoint_path', 'save/res_gru/', 'folder to save checkpoints into (empty = this folder)')
  ...
  if (iter ~=0 and (iter % opt.save_checkpoint_every == 0 or iter == opt.max_iters)) then
    ...
    if best_score == nil or current_score > best_score then
      ...
      if iter > 0 then -- dont save on very first iteration
        ...
        torch.save(checkpoint_path .. '_' ..iter ..'.t7', checkpoint)
        print('wrote checkpoint to ' .. checkpoint_path .. '_' ..iter ..'.t7')
      end
    end
  end
