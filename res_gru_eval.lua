...
require 'utils.NT2_ORG_DataLoaderRaw'
...
-- Input paths
cmd:option('-model','save/res_gru/model_id_160000.t7','path to model to evaluate')
-- Basic options
cmd:option('-batch_size', 1, 'if > 0 then overrule, otherwise load from checkpoint.')
cmd:option('-num_images', 5000, 'how many images to use when periodically evaluating the loss? (-1 = all)')
...
cmd:option('-beam_size', 4, 'used when sample_max = 1, indicates number of beams in beam search. Usually 2 or 3 works well. More is not better. Set this to 1 for faster runtime but a bit worse performance.')
cmd:option('-temperature', 1.0, 'temperature when sampling from distributions (i.e. when sample_max = 0). Lower = "safer" predictions.')
-- For evaluation on a folder of images:

cmd:option('-image_folder', './test_image', 'If this is nonempty then will predict on the images in this folder path')
